...
> **[Important Notice]** We have released our new data framework called `frictionless`. This framework is logical continuation of `tabulator` that was extended to be a complete data solution. The change in not breaking for the existing software so no actions are required. Please read the [Migration Guide](https://github.com/frictionlessdata/frictionless-py/blob/master/docs/target/migration-guide/README.md) to migrate from `tabulator` to Frictionless Framework.
